...
export function createBot(config: eds.ConfigExemplar): eds.KnownRuntimeProperties
{
    ...
    runtimeStorage.set("loader",                new Loader(config.commandsPath, false, config.doNotLoadFilesStartingWith, config.onlyLoadFilesStartingWith, config.includeBuiltinCommands));
